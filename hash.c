...
#define HASH_IMPLEMENT(T)                                                                          \
    ...
    struct Hash##T {                                                                               \
        ...
        Bucket##T array[];                                                                         \
                                                                                                   ...
                                                                                                   \
    void hash_##T##_zero(struct Hash##T *map) {                                                    \
        map->collisions = 0;                                                                       \
        map->length = 0;                                                                           \
    ...
    struct Hash##T *hash_##T##_create(uint32 length) {                                             \
        struct Hash##T *map;                                                                       \
        size_t size;                                                                               \
        uint32 capacity = 1;                                                                       \
        uint32 power = 0;                                                                          \
        ...
        return map;                                                                                \
                                                                                                   ...
                                                                                                   \
    struct Hash##T *hash_##T##_balance(struct Hash##T *old_map) {                                  \
        ...
        for (uint32 i = 0; i < old_map->capacity; i += 1) {                                        \
            ...
            if (iterator->key) {                                                                   \
                ...
                hash_##T##_insert_pre_calc(new_map, iterator->key, hash, index,                    \
                                           HASH_ITERATOR_VALUE);                                   \
            }                                                                                      \
                                                                                                   \
            while (iterator->next) {                                                               \
        ...
        return new_map;                                                                            \
                                                                                                   ...
                                                                                                   \
    void hash_##T##_destroy(struct Hash##T *map) {                                                 \
        arena_destroy(map->arena);                                                                 \
        xmunmap(map, HASH_MAP_SIZE(map));                                                          \
        return;                                                                                    \
    }                                                                                              \
                                                                                                   \
    INLINE bool hash_##T##_insert(struct Hash##T *map, char *key, uint32 key_length,               \
                                  uint32 value) {                                                  \
        ...
        return hash_##T##_insert_pre_calc(map, key, hash, index, value);                           \
                                                                                                   ...
                                                                                                   \
    bool hash_##T##_insert_pre_calc(struct Hash##T *map, char *key, uint32 hash, uint32 index,     \
                                    uint32 value) {                                                \
        ...
        return true;                                                                               \
                                                                                                   ...
                                                                                                   \
    INLINE void *hash_##T##_lookup(struct Hash##T *map, char *key, uint32 key_length) {            \
        uint32 hash = hash_function(key, key_length);                                              \
        uint32 index = hash_normal(map, hash);                                                     \
        return hash_##T##_lookup_pre_calc(map, key, hash, index);                                  \
    }                                                                                              \
                                                                                                   \
    void *hash_##T##_lookup_pre_calc(struct Hash##T *map, char *key, uint32 hash, uint32 index) {  \
        Bucket##T *iterator = &(map->array[index]);                                                \
                                                                                                   \
        if (iterator->key == NULL)                                                                 \
            return NULL;                                                                           \
        ...
        return NULL;                                                                               \
                                                                                                   ...
                                                                                                   \
    INLINE bool hash_##T##_remove(struct Hash##T *map, char *key, uint32 key_length) {             \
        uint32 hash = hash_function(key, key_length);                                              \
        uint32 index = hash_normal(map, hash);                                                     \
        return hash_##T##_remove_pre_calc(map, key, hash, index);                                  \
    }                                                                                              \
                                                                                                   \
    bool hash_##T##_remove_pre_calc(struct Hash##T *map, char *key, uint32 hash, uint32 index) {   \
        Bucket##T *iterator = &(map->array[index]);                                                \
                                                                                                   \
        if (iterator->key == NULL)                                                                 \
            return false;                                                                          \
        ...
        return false;                                                                              \
                                                                                                   ...
                                                                                                   \
    void hash_##T##_print_summary(struct Hash##T *map, char *name) {                               \
        ...
        return;                                                                                    \
                                                                                                   ...
                                                                                                   \
    void hash_##T##_print(struct Hash##T *map, bool verbose) {                                     \
...
main(void) {
    ...
    for (int i = 0; i < NSTRINGS; i += 1) {
        assert(hash_map_insert(original_map, strings[i].s, strings[i].length, strings[i].value));
    }
