...
#define HASH_IMPLEMENT(T)                                                      \
    ...
    struct Hash##T *hash_##T##_balance(struct Hash##T *old_map) {              \
        ...
        for (uint32 i = 0; i < old_map->capacity; i += 1) {                    \
            ...
            if (iterator->key) {                                               \
                ...
                hash_##T##_insert_pre_calc(new_map, iterator->key, hash,       \
                                           index, HASH_ITERATOR_VALUE);        \
            }                                                                  \
                                                                               \
            while (iterator->next) {                                           \
    ...
    void hash_##T##_destroy(struct Hash##T *map) {                             \
        ...
        return;                                                                \
                                                                               ...
                                                                               \
    INLINE bool hash_##T##_insert(struct Hash##T *map, char *key,              \
        ...
        return hash_##T##_insert_pre_calc(map, key, hash, index, value);       \
                                                                               ...
                                                                               \
    bool hash_##T##_insert_pre_calc(struct Hash##T *map, char *key,            \
        ...
        return true;                                                           \
                                                                               ...
                                                                               \
    INLINE void *hash_##T##_lookup(struct Hash##T *map, char *key,             \
                                   uint32 key_length) {                        \
        uint32 hash = hash_function(key, key_length);                          \
        uint32 index = hash_normal(map, hash);                                 \
        return hash_##T##_lookup_pre_calc(map, key, hash, index);              \
                                                                               ...
                                                                               \
    void *hash_##T##_lookup_pre_calc(struct Hash##T *map, char *key,           \
        ...
        return NULL;                                                           \
                                                                               ...
                                                                               \
    INLINE bool hash_##T##_remove(struct Hash##T *map, char *key,              \
                                  uint32 key_length) {                         \
        uint32 hash = hash_function(key, key_length);                          \
        uint32 index = hash_normal(map, hash);                                 \
        return hash_##T##_remove_pre_calc(map, key, hash, index);              \
                                                                               ...
                                                                               \
    bool hash_##T##_remove_pre_calc(struct Hash##T *map, char *key,            \
                                    uint32 hash, uint32 index) {               \
        Bucket##T *iterator = &(map->array[index]);                            \
                                                                               \
        if (iterator->key == NULL)                                             \
